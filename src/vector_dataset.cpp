...
namespace nvdb {
  ...
  const void* VectorDataset::data_ptr() const {
    if (dtype_ == static_cast<uint32_t>(DType::Float32)) return (const void*)vectors_f32_;
    if (dtype_ == static_cast<uint32_t>(DType::Float16)) return (const void*)vectors_f16_;
    // if (dtype_ == static_cast<uint32_t>(DType::Int8)) return (const void*)vectors_i8_;
    return nullptr;
  }
